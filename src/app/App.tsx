...
export function App() {
  ...
  return <div className={`app-shell ${sidebarOpen ? 'sidebar-is-open' : ''}`}>
    ...
    <main className="chat-canvas" id="main-content">
      ...
      {view === 'browser' ? <BrowserView browser={browser} assistantDrive={!!settings.assistantBrowserDrive} onAssistantDrive={enabled => { void store.saveSettings({ assistantBrowserDrive: enabled }); }} /> : view === 'services' ? <ServicesView controller={services} onSettings={() => setSettingsOpen(true)} /> : view === 'chat' ? <>
